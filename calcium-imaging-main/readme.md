概述
===========================
calcium-imaging-main/是 Calcium-Imaging-in-Decision-Making-Mice项目的核心分析与代码仓库。它包含从小鼠决策行为钙成像数据的预处理、特征提取，到多种机器学习模型（MLP, CNN, LSTM, SVM）的定义、训练、评估与结果可视化的完整流水线。所有生成的中间数据、最终数据集、预训练模型及分析图表均存储于此目录。
